...
void Analyse::Loop(string fnamein, string fnameout, bool isData, double xsec, int itype, double nevents)
{
  ...
  hmap["mass"] = new TH1F("mass","mass",20,80,100);
  hmap["sieie"] = new TH1F("sieie","sie", 35, 0.005, 0.012);
  hmap["r9"] = new TH1F("r9","r9", 100, 0.2, 1.1);
  hmap["hOe"] = new TH1F("hOe","hOe", 20,0.,0.05);
  hmap["chIso"] = new TH1F("chIso","chIso", 50,0,100);
  hmap["phIso"] = new TH1F("phIso","phIso", 50,0,100);
  ...
  //for (Long64_t jentry=0; jentry<100000;jentry++) {
    ...
    int nGoodPho = 0;
    int phoArr[100];
    int* phoIndArr = selectPho(nGoodPho, phoArr);
	...
	if( mass>80 && mass<100 && tmpmassDiff>massDiff )
	  {
	    foundPair = true;
	...
	cout<<"Mass is "<<mass<<endl;
      ...
      ///fill the probe photon quantities
      
      hmap["mass"]->Fill(mass, weight);
      hmap["sieie"]->Fill(phoSigmaIEtaIEtaFull5x5->at(phoInd), weight);
      hmap["r9"]->Fill(phoR9Full5x5->at(phoInd), weight);
      hmap["hOe"]->Fill(phoHoverE->at(phoInd), weight);
      hmap["chIso"]->Fill(phoPFChIso->at(phoInd), weight);
      hmap["phIso"]->Fill(phoPFPhoIso->at(phoInd), weight);
      hmap["neIso"]->Fill(phoPFNeuIso->at(phoInd), weight);

      hmap["pt"]->Fill(pho->Pt(), weight);
      hmap["eta"]->Fill(pho->Eta(), weight);
...
int* Analyse::selectPho(int &nGoodPho, int *phoIndArr){
