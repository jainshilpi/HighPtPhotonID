...
void plot(){
  ...
  TFile *fdata = TFile::Open("minitree_data.root");
  TFile *f   = TFile::Open("minitree_dy.root");
  ...
  while ((key = (TKey*)next())) {
    ...
    hmc->SetMarkerColor(histLineColor);
    hmc->SetMarkerSize(0);
    ...
    cout<<"drawing both"<<endl;
    //hdata->SetMarkerSize(2);
    hmc->Draw("LF2BAR");
    hdata->Draw("PE1sames");
    
    cout<<"drawn both"<<endl;
    ...
    */
    cout<<"diving hratio"<<endl;

    hratio->Divide(hdata,hmc);

    cout<<"divided hratio"<<endl;
